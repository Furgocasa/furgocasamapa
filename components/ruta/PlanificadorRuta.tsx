...
export default function PlanificadorRuta({ vistaMovil = 'ruta', onRutaCalculada }: PlanificadorRutaProps = {}) {
  ...
  const [showAuthModal, setShowAuthModal] = useState(false) // Login inline al guardar sin cuenta
  const [showAddFavsModal, setShowAddFavsModal] = useState(false) // Ofrecer añadir áreas de la ruta a favoritos
  ...
  const handleSaveRuta = async () => {
    ...
    try {
      ...
      // Verificar que el usuario esté autenticado: si no, login inline
      // sin abandonar la página (la ruta calculada no se pierde)
      ...
      track('route_save', {
        event_data: {
          distancia_km: distanciaTotal / 1000,
          num_paradas: waypoints.length,
          num_areas: areasEnRuta.length,
        },
      })

      // Cerrar el loop: ofrecer guardar las áreas de la ruta como favoritos
      if (areasEnRuta.length > 0) {
        setTimeout(() => setShowAddFavsModal(true), 300)
      } else {
        setTimeout(() => {
  ...
  /** Tras autenticarse desde el modal de guardado, retomar el guardado */
  const handleAuthSuccess = async (loggedUser: any) => {
    ...
    // Retomar el guardado de la ruta automáticamente
    void handleSaveRuta()
  }

  /** Añade todas las áreas de la ruta a favoritos (ignora duplicados) */
  const handleAddRouteAreasToFavorites = async () => {
    setAddingFavs(true)
    try {
      const supabase = createClient()
      const { data: { session } } = await supabase.auth.getSession()
      ...
      let added = 0
      for (const area of areasEnRuta) {
        const { error } = await (supabase as any)
          .from('favoritos')
          .insert({ user_id: session.user.id, area_id: area.id })
        if (!error) {
          added++
          track('area_favorite', { area_id: area.id, event_data: { origen: 'ruta' } })
        }
      }

      setShowAddFavsModal(false)
      showToast(
        added > 0
          ? `❤️ ${added === 1 ? '1 área añadida' : `${added} áreas añadidas`} a tus favoritos`
          : 'Estas áreas ya estaban en tus favoritos',
        'success'
      )
    } catch (error: any) {
      console.error('Error añadiendo favoritos de ruta:', error)
      setShowAddFavsModal(false)
      showToast(t('ruta_ok_saved'), 'success')
    } finally {
      setAddingFavs(false)
  ...
  return (
    <div className="h-full w-full min-h-0 min-w-0 flex overflow-hidden">
      ...
      {showAuthModal && (
        <AuthModal
          title="Guarda esta ruta en tu cuenta"
          subtitle="Crea una cuenta gratis: tu ruta calculada no se pierde y podrás reabrirla en cualquier dispositivo."
          onClose={() => setShowAuthModal(false)}
          onSuccess={handleAuthSuccess}
        />
      )}

      {/* Ofrecer añadir las áreas de la ruta a favoritos tras guardar */}
      {showAddFavsModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-xl shadow-2xl max-w-md w-full p-6 text-center">
            ...
            <h3 className="text-xl font-bold text-gray-900 mb-2">{t('ruta_ok_saved')}</h3>
            <p className="text-gray-600 mb-6">
              La ruta pasa por <strong>{areasEnRuta.length}</strong> {areasEnRuta.length === 1 ? 'área' : 'áreas'}.
              ¿Las añadimos a tus favoritos para tenerlas a mano durante el viaje?
            </p>
            <div className="space-y-2">
              <button
                onClick={handleAddRouteAreasToFavorites}
                disabled={addingFavs}
                className="w-full py-3 bg-[#0b3c74] text-white rounded-lg font-semibold hover:bg-[#0d4a8f] transition-all disabled:opacity-50"
              >
                {addingFavs ? 'Añadiendo...' : `❤️ Sí, añadir ${areasEnRuta.length === 1 ? 'el área' : `las ${areasEnRuta.length} áreas`}`}
              </button>
              <button
                onClick={() => setShowAddFavsModal(false)}
