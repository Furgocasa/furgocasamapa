...
| Huecos península | área + camping + **aparcamiento / estacionamento** | `scripts/scripts_empresas/import-iberia-gaps.ts` | `npm run import:iberia:gaps` → `--from-report --import` |
...
| Campings Murcia + Alicante | **camping** / càmping (16 disparos) | mismo script `--region=murcia-alicante` | `npm run import:levante:campings` → `import:levante:campings:import` |
...
| Campings Teruel–Guadalajara | **camping** | `--region=teruel|guadalajara` | `npm run import:campings:teruel` / `import:campings:guadalajara` |
