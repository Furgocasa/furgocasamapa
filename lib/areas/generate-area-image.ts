...
export function buildAreaImagePrompt(area: AreaImageInput, tieneMapa = false): string {
  ...
  return [
    ...
    'VEHICLE ACCESS (mandatory, realistic): the camper must look like it drove in. Show a clear, wide entrance or driveway connecting the pitch to a track or road, with enough turning space for a long van. If there is a stone, timber or curb border, it MUST be open on the access side — never a closed rectangle that traps the vehicle. No rocks, logs, fences, ditches or posts blocking the way in. Wheels sit naturally on the ground, not floating or spawned inside a sealed pad.',
